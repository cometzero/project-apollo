...
still fetched by `meta-zephyr` and used from the Yocto `${WORKDIR}/git` unpack
tree.

Local builds follow the same model: `./local-build.sh zephyr` uses the Yocto
`zephyr-demos-cl1` unpack tree for `modules/`, `bootloader/`, and `tools/`.
Set `ZEPHYR_DEPS_SRC` only when using a custom Yocto-unpacked dependency root.
