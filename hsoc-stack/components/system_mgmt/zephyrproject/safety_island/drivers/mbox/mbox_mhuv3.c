...
struct mbox_mhuv3_data {
	...
	/** Array of channels */
	/**
	 * The total number of channels should be increased when the rest of the
...
static int mbox_mhuv3_dbe_channels_init(const struct device *dev, uint32_t *base_ch_idx)
{
	...
	/**
	 * This condition is changed from upstream driver to tackle a bug.
	 * Note that we need to upstream this change.
	 * Change: < data->num_chans -->  <= data->num_chans
	 */
	...
	/**
	 * sNeed upstream change to driver.
	 * Change: &data->channels[*base_ch_idx] --> &data->channels[*base_ch_idx][0]
	 * Reason: 'channels' is 2D array. &data->channels[*base_ch_idx] produces
	 * a pointer to an array, not a pointer to a channel element.
	 */
	chan = &data->channels[*base_ch_idx][0];
...
static int mbox_mhuv3_sender_mtu_get(const struct device *dev)
{
	...
	/**
	 * The maximum transmit units depends on the channel extension.
	 * The type of extension, channel and doorbell should be
	 * extracted from a struct passed to this function from device tree.
...
/**
 * Need upstream update
 * Change: Initialisation of static const struct mbox_mhuv3_config mhuv3_cfg
 * Reason: Compiler warning DT_INST_REG_ADDR(n) expands to a u32 literal.
 * Initializing a pointer field from a plain integer triggers a warning
 */
#define MHUV3_INIT(n)							\
									\
...
static const struct mbox_mhuv3_config mhuv3_cfg_##n = {			\
	.ctrl = (struct ctrl_page *)DT_INST_REG_ADDR(n),			\
	.pbx = (struct mhuv3_pbx_frame_reg *)DT_INST_REG_ADDR(n),						\
	.cmb_irq_config = mbox_mhuv3_cmb_irq_config_##n,					\
};									\
									\
