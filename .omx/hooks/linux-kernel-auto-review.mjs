...
const KERNEL_EXT_RE =
  /\.(c|h|S|rs|dts|dtsi|yaml|yml|rst|patch|cfg|scc|bb|bbappend|inc|conf)$/;
const KERNEL_BASENAME_RE = /(^|\/)(Kconfig|Kbuild|Makefile)$/;
...
function isKernelFilename(path) {
  return KERNEL_EXT_RE.test(path) || KERNEL_BASENAME_RE.test(path);
}

function isLinuxKernelReviewPath(value) {
  ...
  if (!isKernelFilename(path)) {
    return false;
  }

  if (path.startsWith("sw-ref-stack/components/primary_compute/linux_drivers/")) {
    return path.includes("/src/");
  }

  if (
    path.startsWith(
      "sw-ref-stack/yocto/meta-arm-auto-solutions/recipes-kernel/linux/",
    ) ||
    path.startsWith(
      "sw-ref-stack/yocto/meta-arm-auto-solutions/recipes-kernel/hipc-mod/",
    ) ||
    path.startsWith(
      "sw-ref-stack/yocto/meta-arm-auto-solutions/recipes-kernel/pfdi-misc-mod/",
    ) ||
    path.startsWith(
      "arm-zena-css/yocto/meta-zena-css-bsp/recipes-kernel/linux/",
    )
  ) {
    return true;
  }

  if (path.startsWith("layers/") && path.includes("/recipes-kernel/")) {
    return true;
  }
...
function addPath(paths, path, action = "touched") {
  ...
  if (!isLinuxKernelReviewPath(normalized)) {
    return;
  }
...
function extractPathsFromText(text) {
  ...
  if (looksWriteLikePayload(body)) {
    const tokenRe =
      /(?:^|[\s"'`(])((?:\.\/)?(?:arm-zena-css|sw-ref-stack|layers)\/[A-Za-z0-9._@%+:/=-]+?(?:\.(?:c|h|S|rs|dts|dtsi|ya?ml|rst|patch|cfg|scc|bbappend|bb|inc|conf)|\/(?:Kconfig|Kbuild|Makefile)))(?:$|[\s"'`,)])/gm;
...
async function queueReview(event, sdk) {
  ...
  await sdk.log.warn("linux kernel review queued", {
    skill: "$linux-kernel-review",
...
function buildPrompt(paths) {
  ...
  return [
    "$linux-kernel-review Auto-review newly created or changed Linux kernel source or kernel metadata files using the Sashiko-inspired protocol.",
    "",
    bullets + suffix,
    "",
    "Use doc/linux-kernel-source-review.md, read references/sashiko-protocol.md, and report reachability, proven findings, false positives eliminated, static evidence, kernel-tooling, BitBake task, image, and runtime evidence separately.",
  ].join("\n");
}
...
async function maybeSendReviewPrompt(event, sdk) {
  ...
  if (result.ok) {
    await sdk.log.info("linux kernel auto-review prompt sent", logPayload);
  } else {
    await sdk.log.warn("linux kernel auto-review prompt not sent", logPayload);
  }
}
