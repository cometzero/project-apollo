...
현재 F3 r5의 known risk는 freerunning RSE/SCP readiness의 r3 ordering
failure 한 건이다. quiet same-input r4와 current-source r5 pass는 retry
wrapper, fixed sleep, source synchronization 변경 없이 얻었으므로 원인은
supported지만 causally confirmed가 아니다.
...
| 10 | 28 | `deferred-nonessential` | AP PCI MSI-X to ITS physical LPI와 INTx 비교 | QBox opt-in profile은 current F3 r5 Task9에서 PASS. FVP comparison은 immutable first-ECAM abort 때문에 `UNSUPPORTED`/`NOT_COMPARABLE`로 유지 | 3,5,27 | QBox-only PASS를 FVP parity나 default deploy evidence로 과장할 위험 | `python3 scripts/test/run_gic720ae_pcie_irq_validation.py --self-test-negative tests/fixtures/gic720ae/pcie-no-irq-delta.json --out-dir <attemptDir>/task-28-negative` | `.omo/evidence/apollo-gic-its/final/F3/cycle2/run-current-r5/result.json` |
