...
| AP interrupt controller | AP GIC at `0x20800000`, redist at `0x20880000`, legacy aliases at `0x20000000` and `0x200c0000`; ITS is present in the direct DTS at `0x20840000`. | QBox needs active redistributor regions for `PC_CPUS_COUNT_DEFAULT=16` and must preserve aliases used by firmware. |
| AP DRAM | Low DRAM starts at `0x80000000`; AP 9.1.1 high DRAM starts at `0x08_8000_0000` / `0x880000000`. Current Arm Zena CSS FVP boot artifacts use the multichip DRAM aperture bank at `0x200_0000_0000`. | QBox direct AP and full-system paths use `0x20000000000` with 2 GiB backing for FVP parity. The AP 9.1.1 `0x880000000` row remains tracked as deferred programmer-model parity, not as the active boot bank. |
...
| --- | --- | --- |
| High DRAM | Direct `ram_1`, full-system `host_ap_dram2`, and direct DTS high-memory cells use `0x20000000000` with 2 GiB backing. | `partial`, because this follows the current FVP boot bank and does not cover the full AP 9.1.1 programmer-model high DRAM span. |
