...
Source: <https://developer.arm.com/documentation/ddi0487/mc/-Part-C-The-AArch64-Instruction-Set/-Chapter-C8-SVE-Instruction-Descriptions/-C8-2-Alphabetical-list-of-SVE-instructions/-C8-2-30-ANDV>

##### C8.2.30 ANDV

Bitwise AND reduction to scalar

This instruction performs a bitwise AND between the elements of the source vector, and places the result in the SIMD&FP scalar destination register. Inactive elements in the source vector are treated as all ones.

###### SVE

###### (FEAT\_SVE || FEAT\_SME)

![](images/1693-C8.2.30-ANDV-img01.svg)
...
var result : bits(esize) = Ones{esize};
for e = 0 to elements-1 do
    if ActivePredicateElement{PL}(mask, e, esize) then
        result = result AND operand[e*:esize];
    end;
end;
V{esize}(d) = result;
```
