...
Source: <https://developer.arm.com/documentation/ddi0487/mc/-Part-C-The-AArch64-Instruction-Set/-Chapter-C5-The-A64-System-Instruction-Class/-C5-5-A64-System-instructions-for-TLB-maintenance/-C5-5-45-TLBI-RVALE1IS--TLBI-RVALE1ISNXS--TLB-Range-Invalidate-by-VA--Last-level--EL1--Inner-Shareable>
...
Configuration
:   This system instruction is present only when FEAT\_TLBIRANGE is implemented and FEAT\_AA64 is implemented. Otherwise, direct accesses to TLBI RVALE1IS, TLBI RVALE1ISNXS are UNDEFINED.
...
![](images/0553-C5.5.45-TLBI-RVALE1IS-TLBI-RVALE1ISNXS-TLB-Range-Invalidate-by-VA-Last-level-EL1-Inner-Shareable-img01.svg)
...
if !(IsFeatureImplemented(FEAT_TLBIRANGE) && IsFeatureImplemented(FEAT_AA64)) then
    ...
    elsif EL2Enabled() && IsFeatureImplemented(FEAT_FGT) && (!HaveEL(EL3) || SCR_EL3().FGTEn == '1') && HFGITR_EL2().TLBIRVALE1IS == '1' then
        AArch64_SystemAccessTrap(EL2, 0x18);
    else
        AArch64_TLBI_RVA(SecurityStateAtEL(EL1), Regime_EL10, VMID(), Broadcast_ISH, TLBILevel_Last, TLBI_AllAttr, X{64}(t));
    end;
...
`When FEAT_XS is implemented TLBI RVALE1ISNXS{, <Xt>}`
:
  ...
  <tr>
   ...
   <td>
    ...
    if !(IsFeatureImplemented(FEAT_TLBIRANGE) && IsFeatureImplemented(FEAT_AA64)) then
        ...
        if EL2Enabled() && (HCR_EL2().TTLB == '1' || HCR_EL2().TTLBIS == '1') then
            AArch64_SystemAccessTrap(EL2, 0x18);
        elsif EL2Enabled() && IsFeatureImplemented(FEAT_FGT) && (!HaveEL(EL3) || SCR_EL3().FGTEn == '1') && IsFeatureImplemented(FEAT_HCX) && (!IsHCRXEL2Enabled() || HCRX_EL2().FGTnXS == '0') && HFGITR_EL2().TLBIRVALE1IS == '1' then
            AArch64_SystemAccessTrap(EL2, 0x18);
        else
            AArch64_TLBI_RVA(SecurityStateAtEL(EL1), Regime_EL10, VMID(), Broadcast_ISH, TLBILevel_Last, TLBI_ExcludeXS, X{64}(t));
        end;
    elsif PSTATE.EL == EL2 then
        AArch64_TLBI_RVA(SecurityStateAtEL(EL1), Regime_EL10, VMID(), Broadcast_ISH, TLBILevel_Last, TLBI_ExcludeXS, X{64}(t));
    elsif PSTATE.EL == EL3 then
