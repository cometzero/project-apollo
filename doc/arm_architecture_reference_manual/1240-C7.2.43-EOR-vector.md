...
Source: <https://developer.arm.com/documentation/ddi0487/mc/-Part-C-The-AArch64-Instruction-Set/-Chapter-C7-A64-Advanced-SIMD-and-Floating-point-Instruction-Descriptions/-C7-2-Alphabetical-list-of-A64-Advanced-SIMD-and-floating-point-instructions/-C7-2-43-EOR--vector->

##### C7.2.43 EOR (vector)

Bitwise exclusive-OR (vector)

This instruction performs a bitwise exclusive-OR between the first source SIMD&FP register and the second source SIMD&FP register, and places the result in the destination SIMD&FP register.
...
![](images/1240-C7.2.43-EOR-vector-img01.svg)
...
let operand1 : bits(datasize) = V{}(m);
let operand2 : bits(datasize) = Zeros{};
let operand3 : bits(datasize) = Ones{};
let operand4 : bits(datasize) = V{}(n);
V{datasize}(d) = operand1 XOR ((operand2 XOR operand4) AND operand3);
```
