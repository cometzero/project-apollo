...
Source: <https://developer.arm.com/documentation/ddi0487/mc/-Part-C-The-AArch64-Instruction-Set/-Chapter-C8-SVE-Instruction-Descriptions/-C8-2-Alphabetical-list-of-SVE-instructions/-C8-2-477-MSB>

##### C8.2.477 MSB

Multiply-subtract to multiplicand

This instruction multiplies the active elements of the first source vector by the corresponding elements of the second source vector, and subtracts the result from elements of the third (addend) vector. The results are destructively placed in the destination and first source (multiplicand) vector. Inactive elements in the destination vector register remain unmodified.

###### SVE

###### (FEAT\_SVE || FEAT\_SME)

![](images/2140-C8.2.477-MSB-img01.svg)
...
let operand3 : bits(VL) = if AnyActiveElement{PL}(mask, esize) then Z{VL}(a) else Zeros{VL};
var result : bits(VL);
