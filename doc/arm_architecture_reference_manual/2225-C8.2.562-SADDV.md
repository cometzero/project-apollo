...
Source: <https://developer.arm.com/documentation/ddi0487/mc/-Part-C-The-AArch64-Instruction-Set/-Chapter-C8-SVE-Instruction-Descriptions/-C8-2-Alphabetical-list-of-SVE-instructions/-C8-2-562-SADDV>

##### C8.2.562 SADDV

Signed add reduction to scalar

This instruction adds all the signed elements of the source vector, and places the result in the SIMD&FP scalar destination register. Narrow elements are first sign-extended to 64 bits. Inactive elements in the source vector are treated as zero.

###### SVE

###### (FEAT\_SVE || FEAT\_SME)

![](images/2225-C8.2.562-SADDV-img01.svg)
...
        let element : integer = SInt(operand[e*:esize]);
        sum = sum + element;
    end;
end;
V{64}(d) = sum[63:0];
