...
def resolve_elf(root: Path, artifact_root: Path, component: Component) -> Path | None:
    elf = first_existing(artifact_root, component.elf_candidates)
    if elf is not None:
        return elf
...
def shared_library_paths(root: Path, artifact_root: Path) -> tuple[Path, ...]:
    qbox_build = artifact_root / "work/qbox-platform"
    candidates = {
        qbox_build,
...
def write_readme(
    ...
    readme = (
        "# Apollo GDB Environment\n\n"
        "Generated by `scripts/setup/setup_local_debug_env.py`.\n\n"
        "QBox, its component plugins, libqemu, TF-M, SCP-firmware, Zephyr, "
        "TF-A, OP-TEE, U-Boot, and Linux "
