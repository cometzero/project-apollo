...
USAGE = "\n".join(
    (
        ...
        "  --fvp                     Select the apollo-fvp backend and machine",
        "  --qbox                    Select the apollo-qvp backend and machine",
        "  --bsp                     Select the nexios-bsp-initramfs image",
        "  --headless                Use the default non-TUI progress display",
        "  --test-profile NAME       Resolve qa-tests/profiles/NAME.yaml",
        ...
        "  --timeout-fvp SECONDS     Boot timeout (FVP 300s, QBox 600s)",
        "  -h, --help                Show this help",
        "",
        "Final result lines:",
        "  RESULT: PASS|FAIL|BLOCKED",
        "  SUMMARY: build/tests/<stamp>/summary.json",
        "  REPORT: build/tests/<stamp>/summary.txt",
        "  JUNIT: build/tests/<stamp>/junit.xml",
        "  LOGS: build/tests/<stamp>/logs",
        "",
        "Result paths:",
        "  manifest.json, selection.json, resolved-profile.yaml, events.jsonl,",
        "  commands.jsonl, status.json, summary.txt, summary.json, junit.xml,",
