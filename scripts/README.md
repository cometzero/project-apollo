...
`./local-build.sh`, `./run_fvp.sh`, and `./run_qbox.sh`.
...
| `run_local_fvp_tmux.sh` | keep | Interactive FVP tmux runner used by `./run_fvp.sh` and `scripts/debug/run_local_fvp_debug.sh`. |
