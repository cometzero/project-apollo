...
def write_gdb_script(
    ...
    lines.extend(
        [
            ...
            "# Use scripts/local_debug_iris.py to set runtime breakpoints.",
        ]
    )
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
Generated by `scripts/setup_local_debug_env.py`.
...
- `scripts/run_local_fvp_debug.sh` starts FVP with an Iris debug server.
  `scripts/local_debug_iris.py` can connect to that server, set breakpoints
...
```bash
scripts/local_debug_iris.py --port 7100 \\
  --manifest {manifest} \\
