...
the helper reads `MACHINE` from `build/conf/local.conf`, loads the matching
Yocto-variable cache, and currently resolves to `apollo-qvp` and
`build/local-apollo-qvp`. An explicit `MACHINE` takes precedence. The built-in
`apollo-fvp` fallback is used only when no active machine can be read or when
...
`build/local-apollo-qvp/yocto-local-build-vars.json` for the active machine,
generated by
...
python3 scripts/build/collect_yocto_local_build_vars.py \
  --output build/local-apollo-qvp/yocto-local-build-vars.json
```
...
python3 scripts/test/audit_local_build_yocto_parity.py \
  --vars build/local-apollo-qvp/yocto-local-build-vars.json \
  --output build/local-apollo-qvp/yocto-local-build-parity-audit.json
```
